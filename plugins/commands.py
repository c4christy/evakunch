...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, DELETE_TIME, PICS
...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('✧ ᴜᴘᴅᴀᴛᴇs​ ✧', url='t.me/filesharebotusers'),
            InlineKeyboardButton('✧ sᴜᴘᴘᴏʀᴛ​ ✧', url='t.me/filesharebotusers')
            ],[
    ...
    dfile = await client.send_cached_media(
            chat_id=message.from_user.id,
            file_id=file_id,
            caption=f_caption,
            protect_content=True if pre == 'filep' else False,
            )
  #  dfile = await dfile.answer("testing..", show_alert=True)
 #   await asyncio.sleep(DELETE_TIME)
 #   await dfile.delete()
    return
